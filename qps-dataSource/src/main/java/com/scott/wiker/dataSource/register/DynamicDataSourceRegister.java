package com.scott.wiker.dataSource.register;

import com.scott.wiker.dataSource.config.DynamicDataSourceContextHolder;
import com.scott.wiker.dataSource.config.DynamicRoutingDataSource;
...
//            log.info("获取配置信息url：{}",defauleDataSourceProperties.get("url"));
            } catch (Exception e) {
                log.error("master数据源url解密错误,使用原字符串读取",e);
...
//            log.info("获取配置信息username：{}",defauleDataSourceProperties.get("username"));
            } catch (Exception e) {
                log.error("master数据源username解密错误,使用原字符串读取", e);
...
//            log.info("获取配置信息password：{}",defauleDataSourceProperties.get("password"));
            } catch (Exception e) {
                log.error("master数据源password解密错误,使用原字符串读取", e);
            ...
            for (int i = 0; i < configs.size(); i++) {
                ...
                try {
                    try {
                        ...
                        log.error(key + "数据源url解密错误,使用原字符串读取", e);
                        defauleDataSourceProperties.put("url", defauleDataSourceProperties.get("url"));
                    }
                    try {
                        defauleDataSourceProperties.put("username", AESUtil.decrypt(defauleDataSourceProperties.get("username").toString()));
                        ...
                        log.error(key + "数据源username解密错误,使用原字符串读取", e);
                        defauleDataSourceProperties.put("username", defauleDataSourceProperties.get("username"));
                    }
                    try {
                        defauleDataSourceProperties.put("password", AESUtil.decrypt(defauleDataSourceProperties.get("password").toString()));
                        ...
                        log.error(key + "数据源password解密错误,使用原字符串读取", e);
                        defauleDataSourceProperties.put("password", defauleDataSourceProperties.get("password"));
                    }
                    // 绑定参数
